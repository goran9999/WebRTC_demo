...
closeRoom.addEventListener('click',function(){
   ...
   document.getElementById("chat-div").style.display="none";
    document.getElementById("active-meetings").style.display="block";
...
createButton.addEventListener('click',function(){
    ...
    if(roomInput.value==''){
       ...
       document.getElementById("videoButtons").style.display="inline";
...
socket.on("created",function(roomName){
    ...
    //btns.push(myButton);
       ...
       activeCalls.push(newTextBox.innerText);
    ...
    e.button.addEventListener('click',function(){
      ...
      navigator.getUserMedia({
          ...
          audio:false
            ...
            userStream=stream;
            connRoom=activeCalls[order];
            myRoom=activeCalls[order];
            socket.emit("ready",/*activeRooms.value*/activeCalls[order],socket.id);
            myVideo.srcObject=stream;
            myVideo.onloadedmetadata=function(e){
                myVideo.play();
            }
...
socket.on("ready",function(socketid,roomSize){
    ...
    if(!creator){
                ...
                rtcPeerConnection.createOffer().then((offer)=>{
                    rtcPeerConnection.setLocalDescription(offer);
                    console.log(offer);
                    socket.emit("offer",offer,/*activeRooms.value*/activeCalls[order],socketid,socket.id,type);
                });
            ...
            rtcPeerConnection.onicecandidate=OnIceCandidate2;
             ...
             rtcPeerConnection.onremovestream=function(){
               ...
               await rtcPeerConnection.setLocalDescription(answer);
                    ...
                    console.log(answ);
               await socket.emit("answer",answ,/*activeRooms.value*/activeCalls[order],socketid2)
                });
        }
...
function OnIceCandidate2(event){
    if(event.candidate){
    socket.emit("candidate",event.candidate,/*activeRooms.value*/activeCalls[order]);
    }
}
